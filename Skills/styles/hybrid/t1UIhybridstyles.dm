obj/Skills/Buffs/NuStyle/MortalUIStyles// ~~ Angel-taught basic forms ~~  unlocked via tier progression ~~
	Mortal_Instinct_Sword
		...
		SwordClass = "Medium"
		passives = list("Instinct" = 1, "Flow" = 1.5, "Parry" = 1, "Deflection" = 1, "Like Water" = 1.5, "Momentum" = 1, "PUSpike" = 10,)
		...
		adjust(mob/p)
			...
			if(p.AscensionsAcquired==3)
				if(p.isRace(ANGEL))
					...
					StyleOff = 1.15
				if(p.isRace(CELESTIAL))
					passives = list("Instinct" = 1, "LikeWater" = 1.5, "Muscle Power" = 3, "Grippy" = 2.5, "Scoop" = 2, "Iron Grip" = 2.5, "Momentum" = 2.5, "Flow" = 1.5, "Deflection" = 0.5, "Reversal" = 1.5,"UnarmedDamage"=2)
					StyleStr = 1.6
					StyleEnd = 1.4
					StyleDef = 1.15
					StyleSpd = 1.15
